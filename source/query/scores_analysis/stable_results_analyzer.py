...
class StableResultsAnalyzer(object):
    def __init__(self, results_analyzer):
        self.__logger = logging.getLogger('endor.machine_learning.stable_stats_calculator')
        self.__random_scorer = results_analyzer.random_scorer()
        self.__hitrate_calculator = results_analyzer.hitrate_calculator()
        self.__lift_calculator = results_analyzer.lift_calculator()
        self.__recall_calculator = results_analyzer.recall_calculator()
        self.__auc_calculator = results_analyzer.auc_calculator()
    ...
    def calc_stats(self, scores_df, ground, whites, universe):
        ...
        for scorer_name in scorer_names:
            scores_with_ground_df['ind'] = scores_with_ground_df.index
            sorted_scores_df = scores_with_ground_df.sort(columns=[scorer_name, 'ind'], ascending=[False, True])
            sorted_scores_df = sorted_scores_df.drop(['ind'], axis=1)
