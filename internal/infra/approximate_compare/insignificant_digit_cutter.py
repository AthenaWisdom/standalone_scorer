# see https://github.com/snakile/approximate_comparator/blob/master/insignificant_digit_cutter.py
import numpy as np
def cut_insignificant_digits(number, places):
    ...
    if  type(number) != float and type(number)!=np.float64: return number
...
def cut_insignificant_digits_recursively(obj, places):
    ...
    t = type(obj)
    if t == float or t==np.float64: return cut_insignificant_digits(obj, places)
